...
NonSecure_funcptr pfNonSecure_OLED_On = (NonSecure_funcptr)0;
NonSecure_funcptr pfNonSecure_OLED_Off = (NonSecure_funcptr)0;

__attribute__((cmse_nonsecure_entry))
int32_t Secure_OLED_On_callback(NonSecure_funcptr *callback)
{
    pfNonSecure_OLED_On = (NonSecure_funcptr)__builtin_bit_cast(__typeof__(callback), (__builtin_bit_cast(uintptr_t, callback) & ~(uintptr_t)1));
    return 0;
}
...
{
    pfNonSecure_OLED_Off = (NonSecure_funcptr)__builtin_bit_cast(__typeof__(callback), (__builtin_bit_cast(uintptr_t, callback) & ~(uintptr_t)1));
    return 0;
}
...
uint32_t Get_EncryptedDigitallySignedData(t_digitallySignedData *dsd)
{
 uint32_t ret;
 static uint32_t ticks;
 __attribute__((aligned(4))) uint8_t plainBPM[16] =
 {
...
int32_t printSecure(char *string, void *ptr, uint8_t val) {
  ...
  if (ptr == 0 && val == 0) printf("%s\n",string);
  if (ptr == 0 && val != 0) printf(string,val);
 if (ptr != 0 && val == 0) printf(string,ptr);
