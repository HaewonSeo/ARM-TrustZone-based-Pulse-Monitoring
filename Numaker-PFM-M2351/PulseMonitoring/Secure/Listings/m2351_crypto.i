# 1 "Crypto/M2351_crypto.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 358 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Crypto/M2351_crypto.c" 2
...
typedef __attribute__((cmse_nonsecure_call)) void (*NonSecure_funcptr)(uint32_t);
...
void M2351_Crypto_Init(uint8_t channel, uint8_t modeAES)
{
  ...
  AES_Open(((CRPT_T *) ((0x40000000UL) + 0x32000UL)), channel, modeAES, 1UL, 0UL, 3UL);
}
...
void M2351_Encrypt_Data(uint8_t channel, uint8_t InputData[], uint8_t OutputData[], uint32_t bytes)
{
 ...
 AES_SetDMATransfer(((CRPT_T *) ((0x40000000UL) + 0x32000UL)), channel, (uint32_t)InputData, (uint32_t)OutputData, bytes);
# 180 "Crypto/M2351_crypto.c"
 g_AES_done = 0;
...
void M2351_ECC_GenerateKey(char *priKey, char *gKey1, char *gKey2)
{
  ...
  char Qx[] = "1da18eaaa64fdd781bf699feb935744f18f1fa0efac1b255";
  char Qy[] = "0bbeb7ba3ba9d4e6b8aa5738ddef4cd15ce400611094d319";
  ...
  int32_t i;
   ...
   printf("+---------------------------------------------+\n");
    ...
    if(ECC_GeneratePublicKey(((CRPT_T *) ((0x40000000UL) + 0x32000UL)), CURVE_P_192, priKey, gKey1, gKey2) < 0)
    {
        printf("ECC key generation failed!!\n");
        while(1);
    }
# 394 "Crypto/M2351_crypto.c"
...
static int32_t SHAHash(uint32_t u32Mode, uint32_t *pu32Addr, int32_t size, uint32_t digest[])
{
    ...
    ((CRPT_T *) ((0x40000000UL) + 0x32000UL))->HMAC_CTL = (u32Mode << (8)) | (0x1ul << (23));
    ((CRPT_T *) ((0x40000000UL) + 0x32000UL))->HMAC_DMACNT = size;
    ...
    while(size > 0)
    {
        if(size <= 4)
        {
            ((CRPT_T *) ((0x40000000UL) + 0x32000UL))->HMAC_CTL |= (0x1ul << (5));
        }
        ...
        while((((CRPT_T *) ((0x40000000UL) + 0x32000UL))->HMAC_STS & (0x1ul << (16))) == 0);


        ((CRPT_T *) ((0x40000000UL) + 0x32000UL))->HMAC_DATIN = *pu32Addr;
        pu32Addr++;
        size -= 4;
    }


    while(((CRPT_T *) ((0x40000000UL) + 0x32000UL))->HMAC_STS & (0x1ul << (0)));
...
void M2351_SHA_Hash(uint8_t *msg, uint8_t *hash_msg)
{
   ...
   printf("+---------------------------------------------+\n");
    ...
    printf("\n");
    SHAHash(0UL, (uint32_t *)msg, 16, u32_hash);
# 494 "Crypto/M2351_crypto.c"
...
void M2351_ECDSA_GenerateSignature(uint8_t *msg, char *priKey, char* R, char *S)
{
  ...
  M2351_SHA_Hash(msg, (uint8_t *)hash_msg);
    ...
    if(ECC_GenerateSignature(((CRPT_T *) ((0x40000000UL) + 0x32000UL)), CURVE_P_192, hash_msg, priKey, gK, R, S) < 0)
    {
        printf("ECC signature generation failed!!\n");
        while(1);
    }
# 543 "Crypto/M2351_crypto.c"
...
void M2351_ECDSA_VerificationSignature(char *hash_msg, char *pubKey1, char* pubKey2, char* R, char *S)
{
    ...
    if(ECC_VerifySignature(((CRPT_T *) ((0x40000000UL) + 0x32000UL)), CURVE_P_192, hash_msg, pubKey1, pubKey2, R, S) < 0)
    {
